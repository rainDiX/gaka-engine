...
class GLMesh {
   public:
    template <typename V>
    GLMesh(const std::span<const V>& vertices, const GLShaderProgram* program, DrawingMode drawingMode = TRIANGLES);

    template <typename V>
    GLMesh(const std::span<const V>& vertices, const std::span<const uint>& indices, const GLShaderProgram* program,
...
template <typename V>
GLMesh::GLMesh(const std::span<const V>& vertices, const GLShaderProgram* program, DrawingMode drawingMode)
...
template <typename V>
GLMesh::GLMesh(const std::span<const V>& vertices, const std::span<const uint>& indices, const GLShaderProgram* program,
...
template <typename V>
void GLMesh::update(const std::span<const V>& vertices, const std::span<const uint>& indices) noexcept {
    m_vertexBufferSize = updateBuffer(m_vbo, vertices, m_vertexBufferSize, GL_ARRAY_BUFFER);
