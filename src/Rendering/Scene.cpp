...
Scene::Scene()
    : m_camera(FlyingCamera(glm::vec3(0.0f, 2.0f, 5.0f), glm::vec3(0.0f, 2.0f, 4.0f),
...
const std::vector<std::pair<PointLight, glm::vec3>>& Scene::pointLights() const {
...
const std::unordered_map<std::string, std::unique_ptr<RenderObject>>& Scene::objects() const {
  return m_objects;
}
