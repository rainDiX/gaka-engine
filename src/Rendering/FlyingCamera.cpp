...
float FlyingCamera::fov() const { return fov_; }

void FlyingCamera::setFov(float fov) { fov_ = fov; }
...
void FlyingCamera::strafeBy(float distance) {
  auto strafeVector = glm::normalize(glm::cross(getNormalizedViewVector(), up_)) * distance;
...
glm::mat4 FlyingCamera::getViewMatrix() const { return glm::lookAt(position_, viewPoint_, up_); }
