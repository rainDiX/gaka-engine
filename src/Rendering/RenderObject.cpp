...
template <typename T>
size_t updateBuffer(GLuint handle, const std::vector<T>& buffer, size_t old_buf_size, GLenum target) {
...
void setupAttribPointer(const std::vector<VertexAttribute>& attributes, const gk::gpu::gl::GLShaderProgram& program) {
    for (const auto& attrib : attributes) {
        GLint location = program.getAttributeLocation(attrib.name);
        glVertexAttribPointer(location, attrib.size, GL_FLOAT, GL_FALSE, attrib.stride, attrib.offset);
...
RenderObject::RenderObject(const gk::geometry::Mesh& mesh, const std::shared_ptr<gpu::gl::GLShaderProgram> program,
...
void RenderObject::draw(const glm::mat4& projection_matrix, const glm::mat4& view_matrix, const glm::mat4& model_matrix,
    ...
    for (int i = 0; i < int(lights.size()); i++) {
        ...
        m_program->setUniform("pointLights[" + std::to_string(i) + "].intensity", lights[i].first.intensity);
...
void RenderObject::update(const gk::geometry::Mesh& mesh) {
    ...
    m_vertexBufferSize = updateBuffer(m_vbo, mesh.vertices, m_vertexBufferSize, GL_ARRAY_BUFFER);
    m_vertexBufferSize = updateBuffer(m_ebo, mesh.indices, m_vertexBufferSize, GL_ELEMENT_ARRAY_BUFFER);
}

void RenderObject::setDrawingMode(const DrawingMode mode) { m_drawingMode = mode; }
