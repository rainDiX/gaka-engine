...
RenderObject::RenderObject(const gk::geometry::Mesh& mesh, const std::shared_ptr<gfx::gl::GLShaderProgram> program,
                           const std::vector<gfx::gl::GLTexture>& textures, const std::shared_ptr<Material> material)
    : m_program(program), m_material(material), m_textures(textures) {
    m_mesh = std::make_unique<gfx::gl::GLMesh>(std::span<const geometry::Vertex>{mesh.vertices}, mesh.indices,
                                               m_program.get());
}
...
const std::span<const gfx::gl::GLTexture> RenderObject::textures() const noexcept { return m_textures; }
