...
const std::span<const GLVertexAttribute> GLShaderProgram::attributes() const noexcept {
  return m_attributes;
}
...
void GLShaderProgram::compileFile(const std::string& relativePath,
                                  io::RessourceManager& assetManager,
...
void GLShaderProgram::updateAttributes() noexcept {
  ...
  for (GLint attrib = 0; attrib < active_attrs; ++attrib) {
    ...
    m_attributes.push_back(GLVertexAttribute{.name = name,
                                             .size = num,
                                             .type_enum = gl_type,
                                             .stride = 0,
                                             .offset = (void*)offset,
...
template <>
void GLShaderProgram::setUniform<const glm::vec2*>(const std::string& name,
                                                   const glm::vec2* value) const noexcept {
  glUniform2f(glGetUniformLocation(m_id, name.c_str()), value->x, value->y);
}

template <>
void GLShaderProgram::setUniform<const glm::vec3*>(const std::string& name,
                                                   const glm::vec3* value) const noexcept {
  glUniform3f(glGetUniformLocation(m_id, name.c_str()), value->x, value->y, value->z);
}

template <>
void GLShaderProgram::setUniform<const glm::vec4*>(const std::string& name,
                                                   const glm::vec4* value) const noexcept {
  glUniform4f(glGetUniformLocation(m_id, name.c_str()), value->x, value->y, value->z, value->w);
}

template <>
void GLShaderProgram::setUniform<const glm::mat3*>(const std::string& name,
                                                   const glm::mat3* value) const noexcept {
  glUniformMatrix3fv(glGetUniformLocation(m_id, name.c_str()), 1, GL_FALSE, (const float*)value);
}

template <>
void GLShaderProgram::setUniform<const glm::mat4*>(const std::string& name,
